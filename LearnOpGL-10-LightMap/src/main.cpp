...
int main()
{
    ...
    // --------------------
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "LearnOpGL - 9", NULL, NULL);
    if (window == NULL)
    {
    ...
    int width, height;
    unsigned char *diffuse_map = SOIL_load_image(Assets::container2.c_str(), &width, &height, 0, SOIL_LOAD_RGB);

    // load texture and set mipmap
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, diffuse_map);

    glGenerateMipmap(GL_TEXTURE_2D);

    // Free space from texture 1
    SOIL_free_image_data(diffuse_map);
    ...
    unsigned char *specular_map = SOIL_load_image(Assets::container2_specular.c_str(), &width, &height, 0, SOIL_LOAD_RGB);

    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, specular_map);

    glGenerateMipmap(GL_TEXTURE_2D);

    SOIL_free_image_data(specular_map);
    glBindTexture(GL_TEXTURE_2D, 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glClearColor(0.4f, 0.3f, 0.1f, 0.1f);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        // glUniform1f(mixLoc, mixValue);
        ourShader.Use();

        lightPos.x = 1.0f * 2.0f;
        lightPos.y = 1.0f;
        ...
        glm::vec3 ligthColor = glm::vec3(1.0f);
