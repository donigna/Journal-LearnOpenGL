...
of the data, from here you gain access to all the nodes, meshes, materials, animations or textures.fbm
...
@section textures.fbm Textures

Normally textures.fbm used by assets are stored in separate files, however,
there are file formats embedding their textures.fbm directly into the model file.
Such textures.fbm are loaded into an aiTexture structure.
...
return a file path for embedded textures.fbm in FBX files. To test if it is an embedded texture use
...
2. This applies if aiTexture::mHeight == 0 is fulfilled. Then, texture is stored in a "compressed"
   ...
   required to load these textures.fbm.  aiTexture::mWidth specifies the size of the texture data in
...
<table border="1">
  ...
  <tr>
    ...
    embedded textures.fbm start with '*' followed by an index into aiScene::mTextures.</td>
    <td>See the @ref mat_tex section above. Also see @ref textures.fbm for a more information about texture retrieval.</td>
  </tr>
  ...
  <tr>
    ...
    <td>int</td>
    <td>n/a</td>
    <td>Defines how the input mapping coordinates for sampling the n'th texture on the stack 't' are computed. Usually explicit UV coordinates are provided, but some model file formats might also be using basic shapes, such as spheres or cylinders, to project textures.fbm onto meshes.</td>
  ...
  <tr>
   ...
   <tr>
    ...
    <td>aiVector3D</td>
    <td>n/a</td>
    <td></tt> Defines the base axis to to compute the mapping coordinates for the n'th texture on the stack 't' from. This is not required for UV-mapped textures.fbm. For instance, if <tt>MAPPING(t,n)</tt> is #aiTextureMapping_SPHERE, U and V would map to longitude and latitude of a sphere around the given axis. The axis is given in local mesh space.</td>
    <td>-</td>
  </tr>
...
@section uvwsrc How to map UV channels to textures.fbm (MATKEY_UVWSRC)

The MATKEY_UVWSRC property is only present if the source format doesn't specify an explicit mapping from
textures.fbm to UV channels. Many formats don't do this and assimp is not aware of a perfect rule either.
...
have only one uv channel?
   assign channel 0 to all textures.fbm and break

for all textures.fbm
...
// ---------------------------------------------------------------------------------------
// Evaluate multiple textures.fbm stacked on top of each other
...
We suggest that you use custom materials, because they support multiple textures.fbm (like colormap and normalmap). First of all you
