...
class Mesh : public PointCloud {
 ...
 private:
  ...
  // Texture library for storing non-material textures.fbm used by this mesh, e.g.,
  // textures.fbm containing mesh feature IDs of EXT_mesh_features glTF extension.
  // If the mesh is part of the scene then the textures.fbm are stored in the scene.
  // Note that mesh features contain pointers to non-material textures.fbm. It is
  // responsibility of class user to update these pointers when updating the
  // textures.fbm. See Mesh::Copy() for example.
