...
Status MeshUtils::RemoveUnusedMeshFeatures(Mesh *mesh) {
  ...
  // Remove all features textures.fbm that are not used anymore.

  // First find which textures.fbm are referenced by the mesh features.
  ...
  if (!used_textures.empty() &&
    ...
    return ErrorStatus(
        "Trying to remove mesh features textures.fbm that are not owned by the "
        "mesh.");
  }

  // Remove all unreferenced textures.fbm from the non-material texture library.
