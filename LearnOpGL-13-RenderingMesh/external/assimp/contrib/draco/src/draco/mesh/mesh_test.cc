...
TEST(MeshTest, MeshCopyWithMeshFeatures) {
  ...
  // Add two textures.fbm to the non-material texture library of the mesh.
...
// attributes and textures.fbm are not removed.
TEST(MeshTest, TestDeleteMeshFeatures) {
  // The loaded mesh has several vertex attributes and textures.fbm used by the
  // mesh features.
  const std::unique_ptr<draco::Mesh> mesh =
      draco::ReadMeshFromTestFile("BoxesMeta/glTF/BoxesMeta.gltf");
  ASSERT_NE(mesh, nullptr);
  ...
  // and textures.fbm stay the same.
