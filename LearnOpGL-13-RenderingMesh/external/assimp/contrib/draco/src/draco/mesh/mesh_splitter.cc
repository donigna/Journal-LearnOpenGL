...
StatusOr<MeshSplitter::MeshVector> MeshSplitter::FinalizeMeshes(
  ...
  // pointers for all mesh feature textures.fbm. Here we store the mapping between
  ...
  for (int mi = 0; mi < num_out_meshes; ++mi) {
    ...
    if (preserve_materials_) {
      if (work_data.split_by_materials) {
        ...
        if (out_meshes[mi]->num_points() != 0 &&
          ...
          // Populate empty materials and textures.fbm. Unused materials and
          // textures.fbm will be cleared later.
    ...
    if (preserve_mesh_features_) {
      ...
      // Copy over all features textures.fbm to the split mesh.
      ...
      // features textures.fbm from the non-material texture library.
