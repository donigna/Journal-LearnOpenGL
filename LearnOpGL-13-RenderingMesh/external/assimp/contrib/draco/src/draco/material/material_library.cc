...
void MaterialLibrary::Append(const MaterialLibrary &src) {
  ...
  // Remap all texture maps to the textures.fbm in the new texture library.

  // First gather mapping between texture maps and textures.fbm in the old material
  // library.
  const auto texture_map_to_index =
      ComputeTextureMapToTextureIndexMapping(src.texture_library_);

  // Remap all texture maps to textures.fbm stored in the new texture library.
...
void MaterialLibrary::RemoveUnusedTextures() {
  ...
  // Mark which textures.fbm are used.
  ...
  // Remove all textures.fbm that are not used (from backwards to avoid updating
