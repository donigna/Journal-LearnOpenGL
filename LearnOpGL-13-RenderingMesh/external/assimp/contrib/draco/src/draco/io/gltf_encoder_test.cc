...
TEST_F(GltfEncoderTest, TestEncodeFileFunctions) {
  ...
  // bin filename and the textures.fbm will be in the same directory as the output
  // glTF file.
  ...
  // Test encoding with the gltf and bin filename parameter, the textures.fbm will
  ...
  // the textures.fbm will be in the resource_dir directory.
  const std::string new_resource_dir = output_gltf_dir + "/textures.fbm";
  ...
  const std::string new_png_filename =
      draco::GetTestTempFileFullPath("textures.fbm/sphere_Texture0_Normal.png");
...
TEST_F(GltfEncoderTest, PbrNextExtensions) {
  ...
  // properties and textures.fbm, then decoding it and checking that it matches the
  // original model.
...
TEST_F(GltfEncoderTest, EncodeMeshWithMeshFeaturesWithMultiplePrimitives) {
  ...
  const std::unique_ptr<Mesh> mesh(ReadMeshFromTestFile(file_name));
  ASSERT_NE(mesh, nullptr);
  // All mesh features should share two textures.fbm.
