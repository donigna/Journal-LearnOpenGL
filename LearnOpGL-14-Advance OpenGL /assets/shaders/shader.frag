...
uniform PointLigth pointLights[NR_POINT_LIGHTS];
...
vec3 CalcPointLight(PointLigth light, vec3 normal, vec3 fragPos, vec3 viewDir);
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    return light.ambient * vec3(texture2D(material.diffuse, TexCoords));
}
