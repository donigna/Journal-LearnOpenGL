...
Status GltfDecoder::CopyTextures(T *owner) {
  for (int i = 0; i < gltf_model_.images.size(); ++i) {
    ...
    // Update mapping between glTF images and textures.fbm in the texture library.
...
Status GltfDecoder::AddStructuralMetadataToGeometry(GeometryT *geometry) {
  ...
  // attribute, or property textures.fbm (in the future).
...
Status GltfDecoder::DecodeStructuralMetadata(
  ...
  if (object == extension.end()) {
    // TODO(vytyaz): Extension might contain property textures.fbm, support that.
    return OkStatus();
  }
  const tinygltf::Value &array = object->second;
  if (!array.IsArray()) {
...
Status GltfDecoder::ParseTextureInfo(
  ...
  // Note that tinygltf only parses material textures.fbm and not material extension
  // textures.fbm. This method mimics the behavior of tinygltf's private function
  // ParseTextureInfo() in order for Draco to decode extension textures.fbm.
...
void GltfDecoder::MoveNonMaterialTextures(
  ...
  // TODO(vytyaz): Consider textures.fbm that are both material and non-material.
