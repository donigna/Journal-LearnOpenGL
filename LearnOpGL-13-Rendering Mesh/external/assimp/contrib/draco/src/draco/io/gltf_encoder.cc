...
// Struct to hold texture data. Multiple textures.fbm can reference the same image.
struct GltfTexture {
...
class GltfAsset {
 ...
 private:
  ...
  // Counter for naming mesh feature textures.fbm.
...
Status GltfAsset::EncodeMaterials(EncoderBuffer *buf_out) {
  // Check if we have textures.fbm to write.
...
Status GltfAsset::EncodeMaterialsProperty(EncoderBuffer *buf_out) {
  ...
  for (int i = 0; i < material_library_.NumMaterials(); ++i) {
    ...
    if (metallic && occlusion &&
      ...
      if (metallic->texture() == occlusion->texture()) {
        ...
        // Metallic and occlusion textures.fbm are already combined.
  ...
  if (!textures_.empty()) {
    gltf_json_.BeginArray("textures.fbm");
