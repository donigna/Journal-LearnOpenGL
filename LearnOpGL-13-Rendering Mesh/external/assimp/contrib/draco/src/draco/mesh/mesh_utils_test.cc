...
TEST(MeshUtilsTest, RemoveUnusedMeshFeatures) {
  ...
  // The input mesh should have five mesh features and two features textures.fbm.
  ASSERT_EQ(mesh->NumMeshFeatures(), 5);
  ASSERT_EQ(mesh->GetNonMaterialTextureLibrary().NumTextures(), 2);

  // All of those features and textures.fbm should be used so calling the method
